"""tools/debug_image.py
Herramienta para inspeccionar tokens, líneas reconstruidas y candidatos de monto/fecha.
Usa IMAGE_PATH variable (editable) o TEST_IMAGE env var (relativa a data/ o absoluta).
"""
import os
import sys
...
def main():
    ...
    if test_env:
        if not os.path.isabs(test_env):
            IMAGE_PATH = os.path.join(proj_root, 'data', test_env)
        else:
            IMAGE_PATH = test_env
